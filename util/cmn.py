...
class CollaborativeMemoryNetwork(ModelBase):
    ...
    def _construct(self):
        ...
        neighbor_negative = self._mem_layer(neg_query,
                                            ...
                                            self.config.max_neighbors)[-1].output
        self.negative_output = self._output_module(tf.concat([self._cur_user * self._cur_item_negative, neighbor_negative], axis=1))
    ...
    def _construct_placeholders(self):
        ...
        self.input_neighborhoods = tf.placeholder(tf.int32, [None, None], 'Neighborhood')

        self.input_neighborhood_lengths = tf.placeholder(tf.int32, [None], 'NeighborhoodLengthID')
